...
| `DUPLICATE_BOX_IN_BATCH` | 同一箱子在批量导入中重复出现 |
| `DUPLICATE_TRANSFER` | 同一箱子在同一时间点已有交接记录 |
| `INVALID_TRANSFER_TIME` | 交接时间晚于当前时间 |
| `INVALID_TO_POINT` | 接收点与起点相同 |
| `BATCH_IMPORT_ERROR` | 批量导入过程中发生错误 |
| `CSV_PARSE_ERROR` | CSV文件解析失败 |
| `CSV_MISSING_FIELDS` | CSV缺少必要字段 |
| `CSV_ROW_PARSE_ERROR` | CSV行数据解析失败 |
| `CSV_PROCESS_ERROR` | CSV处理过程中发生错误 |
